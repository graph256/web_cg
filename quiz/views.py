...
def quiz_delete(request, slug, pk):
    ...
    quiz.delete()
    messages.success(request, f'успешно удален.')
...
class QuizTake(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if quizQuestions <= 0:
            messages.warning(request, f'Набор вопросов викторины пуст. попробуй позже!')
        ...
        if self.sitting is False:
            ...
            messages.info(request, f'Вы уже сдавали этот экзамен, и разрешена только одна сдача.')
            return redirect('quiz_index', self.course.slug)
