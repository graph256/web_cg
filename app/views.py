...
def mather_form(request):
    return render(request, 'app/mather.html')


def import_three(request):
    return render(request, 'app/import3d.html')
...
def post_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, (title + ' загружен.'))
...
def delete_post(request, pk):
    ...
    title = post.title
    post.delete()
    messages.success(request, (title + ' удален.'))
    return redirect('home')
...
def dashboard_view(request):
    ...
    count_lecturer = User.objects.filter(is_lecturer=True).count() 
    count_courses = Course.objects.all().count
    count_program = Program.objects.all().count()
    count_quiz = Quiz.objects.all().count()
    context = {
        "count_student": count_student, 
        "count_lecturer": count_lecturer,
