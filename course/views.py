...
def program_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, request.POST.get('title') + ' Программа создана.')
...
def program_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, str(request.POST.get('title')) + ' программа обновлена.')
            return redirect('programs')
    else:
        form = ProgramForm(instance=program)

    return render(request, 'course/program_add.html', {
        'title': "Изменение программы | WebCG",
        'form': form
    })
...
def program_delete(request, pk):
    ...
    title = program.title
    program.delete()
    messages.success(request, 'Программа ' + title + ' удалена.')
...
def course_edit(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (course_name + '(' + course_code + ')' + ' обновлен.'))
            return redirect('program_detail', pk=request.POST.get('program'))
        else:
...
def course_delete(request, slug):
    ...
    course.delete()
    messages.success(request, 'Курс ' + course.title + ' удален.')
...
def edit_allocated_course(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'назначеный курс обновлен.')
            return redirect('course_allocation_view')
    else:
...
def handle_file_upload(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (request.POST.get('title') + ' загружен.'))
...
def handle_file_delete(request, slug, file_id):
    ...
    messages.success(request, (file.title + ' удален.'))
    return redirect('course_detail', slug=slug)
...
def handle_video_upload(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (request.POST.get('title') + ' загружено.'))
            return redirect('course_detail', slug=slug)
    else:
        form = UploadFormVideo()
    return render(request, 'upload/upload_video_form.html', {
        'title': "Видео загружено | WebCG",
        'form': form, 'course': course
    })
...
def handle_video_edit(request, slug, video_slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (request.POST.get('title') + ' обновлен.'))
            return redirect('course_detail', slug=slug)
    else:
...
def handle_video_delete(request, slug, video_slug):
    ...
    messages.success(request, (video.title + ' удалено.'))
    return redirect('course_detail', slug=slug)
# ########################################################
...
def course_registration(request):
    if request.method == 'POST':
        ...
        for s in range(0, len(ids)):
            ...
            obj.save()
            messages.success(request, 'Курс успешно зарегистрироваан!')
