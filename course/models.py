...
LEVEL = (
    # (LEVEL_COURSE, "Level course"),
    (BACHLOAR_DEGREE, "Степень бакалавр"),
...
class Course(models.Model):
    ...
    code = models.CharField(max_length=200, unique=True, null=True, verbose_name=('Код'))
    credit = models.IntegerField(null=True, default=0, verbose_name=('Часы'))
    summary = RichTextUploadingField()
    program = models.ForeignKey(Program, on_delete=models.CASCADE, verbose_name=('Программа'))
    level = models.CharField(max_length=25, choices=LEVEL, null=True, verbose_name=('Уровень'))
    year = models.IntegerField(choices=YEARS, default=0, verbose_name=('Год'))
    semester = models.CharField(choices=SEMESTER, max_length=200, verbose_name=('Семестр'))
    is_elective = models.BooleanField(default=False, blank=True, null=True, verbose_name=('Выборочный курс'))
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            # Если объект еще не сохранен в базе данных, то генерируем slug
...
class UploadVideo(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            # Если объект еще не сохранен в базе данных, то генерируем slug
            self.slug = slugify(self.title, allow_unicode=True)
        super(UploadVideo, self).save(*args, **kwargs)
