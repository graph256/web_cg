...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, f'Что-то не так, пожалуйста, заполните все поля правильно.')
    else:
        form = StudentAddForm(request.POST)
...
def staff_add_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Аккаунт для" + first_name + ' ' + last_name + "создан.")
            return redirect("lecturer_list")
    else:
        form = StaffAddForm()

    context = {
        'title': 'Lecturer Add | WebCG',
        'form': form,
    }
...
def edit_staff(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Lecturer ' + full_name + ' обновлен.')
            return redirect('lecturer_list')
        else:
            messages.error(request, 'Пожалуйста введите правильные данные.')
    else:
    ...
    return render(request, 'accounts/edit_lecturer.html', {
        'title': 'Изменить Преподавателя | WebCG',
        'form': form,
    })
...
def delete_staff(request, pk):
    ...
    lecturer.delete()
    messages.success(request, 'Lecturer ' + full_name + ' удален.')
    return redirect('lecturer_list')
# ########################################################
...
def edit_student(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, ('Студент ' + full_name + ' был обновлен'))
            return redirect('student_list')
        else:
